...
**2026-07-13 — edit-25: the drift goes the OTHER way too — Scribe `word.start` can LAND LATE, so `word.start − 0.075` CLIPS the word onset.** Two segments shipped with clipped onsets (user flagged "For" in "For more AI tips"; a self-check then caught "Okay" too): "For" real fricative onset ~91.60 (RMS ramp) / source `silence_end` 91.665, but Scribe tagged `For` at 91.78 → my start 91.705 cut the "F". "Okay" real onset (`silence_end`) 93.847, Scribe 93.94 → start 93.865 cut the "O". Both are Scribe onset tokens sitting ~0.1–0.2s AFTER the real speech onset (opposite of the dead-air case above). **Fix + the verify that should have caught it pre-ship: after render, probe the first ~50ms RMS of EVERY segment start in the final (`astats` first-frame RMS). A clean onset reads ≤ ~-35 dB (starts in silence and ramps up); a first frame hotter than ~-22 dB means the segment begins mid-word → move the start earlier to `silence_end − 0.06`.** So the robust start rule is: derive from the source `silencedetect` `silence_end` preceding the first word (minus ~0.06 lead), NOT `word.start` — same acoustic-not-Scribe principle as deriving ends from `silence_start`.
